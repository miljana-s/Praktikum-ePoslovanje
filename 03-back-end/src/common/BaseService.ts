...
export default abstract class BaseService<ReturnModel extends IModel> {
    ...
    /* protected async getAllBySizeId<AdapterOptions extends IModelAdapterOptions> (
        id: number,
        options: Partial<AdapterOptions> = { }
    ): Promise<ReturnModel[]|IErrorResponse> {
        return new Promise<ReturnModel[]|IErrorResponse>(async (resolve) => {
            ...
            FROM article
            WHERE article_id IN
              (SELECT  article_sizes.article_id
               FROM article_sizes
               WHERE article.article_id = article_sizes.article_id AND size_id = ?);
            `;
            this.db.execute(sql, [id])
                .then(async result => {
                    const rows = result[0];
